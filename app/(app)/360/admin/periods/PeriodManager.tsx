...
const btnSecondary = "flex items-center gap-2 bg-white border border-slate-200 text-slate-700 font-semibold text-sm px-4 py-2 rounded-xl shadow-sm hover:shadow-sm transition-all duration-75";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5">
        <p className="text-sm font-semibold text-slate-800 mb-3">Create New Period</p>
            ...
            periods.map((p) => (
              <div key={p.id} className={`bg-white border rounded-2xl px-5 py-4 flex items-center justify-between flex-wrap gap-3 ${p.isActive ? "border-amber-200 bg-amber-50/30" : "border-slate-200"}`}>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <button onClick={() => del(p.id)} className="text-slate-300 hover:text-red-500 px-3 py-2 rounded-xl text-xs font-semibold hover:bg-red-50 shadow-sm hover:shadow-sm transition-all duration-75">
                    Delete
                  </button>
                </div>
              </div>
